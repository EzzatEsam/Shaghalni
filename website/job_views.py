...
def apply_job(job_id):
	if current_user.usertype == "Business Owner":
		return (
			render_template_string("PageNotFound {{ errorCode }}", errorCode="404"),
			404,
		)
	...
	if job.user_id == current_user.id:
		...
		for application in applications:
			applications_list.append(
				(application, userSystem.GetUser(application.user_id))
			)
	print("job", job.user_id)
	print(current_user.id)
	print(applications_list)
	return render_template(
		"Jobs/job_expanded.html",
		job=job,
		user=current_user,
		img=img,
...
def get_user_applications():
	...
	return render_template(
		"my_applications.html", user=current_user, applications=applications
	)
...
@job_views.route(
	"jobs/submission/make-submission/<int:application_id>", methods=["GET", "POST"]
)
@login_required
...
def recv_submissions():
	...
	return render_template(
		"jobs/recv_submissions.html", user=current_user, submissions=submissions
	)


@job_views.route(
	"jobs/submission/accept_submission/<int:submission_id>", methods=["POST"]
)
@login_required
