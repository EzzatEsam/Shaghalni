...
def login():
    if request.method == "POST":
        ...
        if user:
            if Passwords(user.password, password).is_same(
                "Logged in successfully!", "Incorrect password, try again."
            ):
...
def sign_up():
    ...
    if request.method == "POST":
        ...
        # Demorgan
            ...
            firstcheck or lastcheck or jobcheck or not (is_good_email and is_good_pass)
        ):
            pass
    ...
    return render_template(
        "sign_up.html", user=current_user, dropdown_options=dropdown_options
    )
...
def change_profile():
    ...
    if request.method == "POST":
        ...
        if len(email) == 0 or not is_good_email:
            ...
            CheckTypicality(email, current_user.email).is_equal()
        ...
        if len(first_name) == 0:
            ...
            CheckLength(2, "First name").is_short(
                first_name, showmsg=not CheckTypicality(first_name, "").is_equal()
            )
            or CheckTypicality(first_name, current_user.first_name).is_equal()
        ):
        ...
        if len(last_name) == 0:
            ...
            CheckLength(2, "Last name").is_short(
                last_name, showmsg=not CheckTypicality(last_name, "").is_equal()
            )
            or CheckTypicality(last_name, current_user.last_name).is_equal()
        ):
        ...
        if not (
            CheckLength(4, "Job Description").is_short(
                about, showmsg=not CheckTypicality(about, "").is_equal()
            )
            or CheckTypicality(about, current_user.about).is_equal()
        ):
            user.about = about
            db.session.commit()
        ...
        if not (
            CheckTypicality(gender, current_user.gender).is_equal()
            or CheckTypicality(gender, "").is_equal()
        ):
            user.gender = gender
            db.session.commit()
            flash("Gender updated Successfully!", category="success")
        if not (
            CheckTypicality(usertype, current_user.usertype).is_equal()
            or CheckTypicality(usertype, "").is_equal()
        ):
    ...
    return render_template(
        "change_profile.html", user=current_user, dropdown_options=dropdown_options
    )
...
def change_password():
    if request.method == "POST":
        ...
        is_old_correct = Passwords(user.password, password).is_same(
...
def get_image(user_id):
    ...
    if user:
        try:
            ...
            try:
                return send_file(
                    f"../instance/Images/{user_id}.png", mimetype="image/jpg"
                )
            except BaseException:
                try:
                    return send_file(
                        f"../instance/Images/{user_id}.gif", mimetype="image/jpg"
                    )
                except BaseException:
                    try:
                        return send_file(
                            f"../instance/Images/{user_id}.jpeg", mimetype="image/jpg"
                        )
                    except BaseException:
                        return send_file(
