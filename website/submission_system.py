...
class SubmissionSystem(metaclass=SingletonMeta):
    ...
    def GetSubmissionsToUser(self, user_id) -> list:
        ...
        """
        submissions = (
            ApplicationSubmission.query.join(
                JobApplication,
                JobApplication.id == ApplicationSubmission.application_id,
            )
    ...
    def MakeSubmission(self, application_id, title, file) -> bool:
        print(
            f"Making a submission for application {application_id} with title {title} "
        )
        ...
        if app and app.job_status == "Accepted":
            ...
            submission = ApplicationSubmission(
                title=title, file_path=path, application_id=application_id
            )
    ...
    def AcceptSubmission(self, submission_id: int, user_id: int) -> bool:
        submission: ApplicationSubmission = ApplicationSubmission.query.filter_by(
            id=submission_id
        ).first()
    ...
    def OnAppDeleted(self, app_id: int):
        ...
        """
        for submission in ApplicationSubmission.query.filter_by(
